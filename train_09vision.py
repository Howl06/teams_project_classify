...
def learning_speed(val_acc: float, mode: str) -> float:
    if mode == "no_speed":
        ...
        return max(1 / (0.02 + val_acc), 15)
    elif mode == "log":
        return max(1 / (0.01 + math.log(val_acc + 1, 2)), 15)
    elif mode == "e":
        return 1 / (0.02 + math.e ** (val_acc - 1))
    elif mode == "root":
        return max(1 / (0.005 + val_acc ** 0.5), 15)
...
def main():
    ...
    experiment.set_name(model_name_set + "_b_" + str(hyper_params["batch_size"]) + "_e_" + str(hyper_params["epochs"]) + "_" + hyper_params["learning_speed"])
    ...
    experiment.add_tags([
                ...
                "class wight",
                "t_learn", 
                #"max(1/(val_acc+0.02)*learning_rate)"
                ])
    ...
    for epoch in range(hyper_params["epochs"]):
        ...
        optimizer.param_groups[0]["lr"] = optimizer.param_groups[0]["lr"] * learning_speed(val_accurate, hyper_params["learning_speed"])
